...
def normalize_batch(image, audio_scale):
    c = image.shape[0]
    ...
    if '-' not in audio_scale:
        if len(audio_scale) > 1:
            rescaled_image = image * float(audio_scale)
        else:
...
class speechDataTest129(Dataset):
    ...
    def prepare_data(self, audio_scale=False):
        ...
        for i, audio_data in enumerate(self.data_raw):
            ...
            soundwave = audio_data[0]
            stft = torch.stft(soundwave, n_fft=self.frequency * 2, hop_length=self.frequency, win_length=self.frequency*2,
                              return_complex=False).to(device)
            stft_last_row = stft[:, -1, :]
            ...
            for j in range(stft_2.shape[2] // self.len_clip):
                ...
                if '-' not in audio_scale:
                    if len(audio_scale) > 1:
                        # print('audio scale in data preparation: ', audio_scale
                        data_clip = data_clip * float(audio_scale)
                else:
                    min_range, max_range = audio_scale.split('-')
                    #        min_range, max_range = audio_scale.split('-')
                    min_range = float(min_range)
                    ...
                    data_clip = data_clip * (max_range - min_range) + min_range
        ...
        # return data, min_value, max_value
            data.append([audio_data, clip_data, stft.shape[2] % self.len_clip, stft_last_row])
        return data
...
class SpeechDataTrain(Dataset):
    ...
    def prepare_data(self):
        ...
        #         break
            ...
            # remove the last row data in stft to keep 128 dimensions
            stft = stft[:, :-1, :]
            # print(stft.shape)
            # print(stft.shape)
        ...
        if len(self.audio_scale) > 1:
            ...
            data, min_value, max_value = normalize_batch(data, self.audio_scale)
...
def reconstruct_audio(audio_data, watermark, model, n_fft=255, attack=None, data_mode='stft', audio_scale='0', data_min=None, data_max=None, model_name='uformer'):
    device = torch.device('cuda')
    ...
    for i, clip in enumerate(clips):
        ...
        if model_name == 'uformer':
            ...
            audio_clip, wm_decode = model(clip, watermark)
        ...
        if len(audio_scale) > 1:
            ...
            if '-' not in audio_scale:
                factor = 1 / float(audio_scale)
                print('factor in evaluation: ', factor)
                audio_clip = audio_clip * factor
            else:
                min_range, max_range = audio_scale.split('-')
                min_range = float(min_range)
                max_range = float(max_range)
                audio_clip = (audio_clip - min_range) / (max_range - min_range)
                audio_clip = audio_clip * (data_max - data_min) + data_min
        ...
        if i != len(clips)-1:
            ...
            if data_mode == 'stft':
                ...
                # print('len_last_clip: ', len_last_clip, len(preds))
                # print('audio_clip: ', audio_clip.shape)
    ...
    # print('watermark, wm_decode', watermark.shape, wm_decode.shape)
    ...
    if attack_params[0] == 'echo_addition':
        ...
        audio_att = awgn(recon_audio.detach().cpu().numpy(), snr=float(attack_params[1]))
        ...
        audio_att = jittering_2(recon_audio.detach().cpu().numpy(), int(attack_params[1]))
    elif attack_params[0] == 'time_scaling':
        audio_att = time_scaling(recon_audio.detach().cpu().numpy(), float(attack_params[1]))
    elif attack_params[0] == 'pitch_scaling':
        audio_att = pitch_scaling(recon_audio.detach().cpu().numpy())
    elif attack_params[0] == 'mp3compress':
        audio_att = mp3compress(recon_audio.detach().cpu().numpy(), attack_params[1])
    elif attack_params[0] == 'closed_loop':
        audio_att = recon_audio
    ...
    # print('audio_att: ', audio_att.shape)
    # plt.plot(audio_att)
    ...
    if data_mode == 'stft':
        ...
        for j in range(audio_att_feat.shape[3] // 128):
            ...
            if len(audio_scale) > 1:
                ...
                # data_clip = data_clip * (0.1 - 0.01) + 0.01
                if '-' not in audio_scale:
                    # print('audio_scale in evaluation:', audio_scale)
                    data_clip = data_clip * float(audio_scale)
                else:
                    min_range, max_range = audio_scale.split('-')
                    min_range = float(min_range)
            ...
            if model_name == 'uformer':
                ...
                wm_decode_att = model.decode(data_clip)
        ...
        for j in range(audio_att_dwt.shape[1] // 128**2):
            ...
            data_clip = data_clip.reshape(1, 128, 128)
            data_clip = data_clip.float()
