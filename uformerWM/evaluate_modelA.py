...
def test(model, wm_ds, test_dl, data_cat='train', result_path=None, attack=None, save_audio=True, data_mode='stft', audio_scale='0', data_max=None, data_min=None):
    ...
    for i, data in enumerate(tqdm(test_dl), 0):
        ...
        try:
            ...
            ori_audio = data[0][0]
            att_audio, recon_audio, watermark, wm_decode, wm_decode_att, mse_loss, wm_loss, wm_loss_att, snr_ori, snr_recon = reconstruct_audio(data, message, model, attack=attack, data_mode=data_mode, audio_scale=audio_scale, data_max=data_max, data_min=data_min, model_name='modelA')
...
def model_test():
    result_path = 'results/modelA-tedlium-13062023_230030'
    # result_path = 'results/modelA-librispeech-13062023_214046' # librispeech
    # result_path = 'results/modelA_audio-librispeech-14062023_212223' # librispeech
    # result_path = 'results/modelA-audio-tedlium-14062023_221438' # librispeech
    ...
    time.sleep(10)
    if data_mode == 'stft':
        dataset_train = SpeechDataTest(data_cat='train', size=30, dataset=opt.dataset)
        dataset_test = SpeechDataTest(data_cat='test', size=30, dataset=opt.dataset)
        if opt.audio_scale:
    ...
    # data_cats = ['train']
    # attacks = ['aac', 'mp3compress', 'resampling', 'pitch_scaling', 'requantization']
    # attacks = ['echo_addition-0.5-0.2', 'amplitude_scaling-0.8','amplitude_scaling-0.9',
