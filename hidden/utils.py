...
from audio_test import SpeechDataTrain, SpeechDataAudio
...
def get_watermark_data_loader(train_options):
    ...
    wm_dl = iter(DataLoader(wm_ds, batch_size=train_options.batch_size, shuffle=True))
    return wm_ds, wm_dl


def get_data_loaders(hidden_config, train_options):
    train_ds = SpeechDataAudio()
    val_ds = SpeechDataAudio()
    

    # train_ds = SpeechDataTrain(data_type='train', dataset='tedlium')
    # val_ds = SpeechDataTrain(data_type='test', dataset='tedlium')
    # train_ds = SpeechDataTrain()

    train_dl = DataLoader(train_ds, batch_size=train_options.batch_size, shuffle=True,
                          drop_last=True)
    val_dl = DataLoader(val_ds, batch_size=train_options.batch_size, shuffle=True,
                          drop_last=True)
